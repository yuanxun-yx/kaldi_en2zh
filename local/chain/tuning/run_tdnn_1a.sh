...
test_sets="dev test"
get_egs_stage=-10
dir=exp/chain/tdnn_tedlium_aishell_1a
# LSTM/chain options
train_stage=-10
xent_regularize=0.1
...
ivector_dir=exp/nnet3${nnet_affix}/ivectors_${train_set}
...
if [ $stage -le 1 ]; then
  echo "$0: extracting AISHELL ivectors using TEDLIUM extractor"
  # We extract iVectors on the speed-perturbed training data after combining
  # short segments, which will be what we train the system on.   
  
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
    $AISHELL_ROOT/exp/nnet3/ivectors_${train_set}/${train_set}_sp_hires_nopitch_max2 \
    $src_ivec_extractor_dir $ivector_dir
  mv $ivector_dir/final.ie.id $ivector_dir/final.ie.id.bak

  # Also extract iVectors for the test data, but in this case we don't need the speed
  # perturbation (sp).
  for data in $test_sets; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 8 \
      $AISHELL_ROOT/data/${data}_hires_nopitch $src_ivec_extractor_dir \
      exp/nnet3${nnet_affix}/ivectors_${data}
  done
fi
...
if [ $stage -le 8 ]; then
  for test_set in dev test; do
    steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
      ...
      --online-ivector-dir exp/nnet3${nnet_affix}/ivectors_$test_set \
